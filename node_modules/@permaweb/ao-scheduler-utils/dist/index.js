...
function loadProcessSchedulerWith({ fetch: fetch2, GATEWAY_URL: GATEWAY_URL2 }) {
  ...
  return async (process2) => {
    return gateway({ query: GET_TRANSACTIONS_QUERY, variables: { transactionIds: [process2] } }).then(path(["data", "transactions", "edges", "0", "node"])).then(findTransactionTags(`Process ${process2} was not found on gateway ${GATEWAY_URL2}`)).then(findTagValue(SCHEDULER_TAG)).then((walletAddress) => {
      if (!walletAddress)
...
function locateWith({ loadProcessScheduler, loadScheduler, cache, followRedirects, checkForRedirect }) {
  return (process2, schedulerHint) => cache.getByProcess(process2).then(async (cached) => {
    if (cached)
      return cached;
    return Promise.resolve().then(async () => {
      ...
      await cache.setByProcess(process2, byProcess, scheduler.ttl);
      return byProcess;
    });
  });
}
...
function connect({ cacheSize = 100, GATEWAY_URL: GATEWAY_URL2 = DEFAULT_GATEWAY_URL, followRedirects = false } = {}) {
  ...
  const loadScheduler = loadSchedulerWith({ fetch, GATEWAY_URL: GATEWAY_URL2 });
  const cache = {
